...
let
  ...
  renderAgent =
    ...
    let
      ...
      ++ lib.optional (tools != null) "tools: ${builtins.toJSON (lib.concatStringsSep ", " tools)}"
      ++ lib.optional (agent.model != null) "model: ${builtins.toJSON agent.model}"
      ...
      ) "extensions: ${builtins.toJSON (lib.concatStringsSep ", " (resolveExtensions agent.extensions))}"
      ...
      ++
        lib.optional (plugin.subagentOnlyExtensions != null)
          "subagentOnlyExtensions: ${builtins.toJSON (lib.concatStringsSep ", " (resolveExtensions plugin.subagentOnlyExtensions))}"
      ++ [ "---" ];
    in
