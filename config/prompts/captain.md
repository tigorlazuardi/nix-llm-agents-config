---
description: Captain — execute or resume approved Fleet state
argument-hint: "<approved fleet.json ref>"
---
Fleet execution request: $@
...
Active contract: `templates/fleet/{fleet,state}.schema.json`, templates, prompts, validator, and consolidated agent frontmatter. Historical design docs are archival; never use them to override this prompt or active schemas.
...
Any of these fails → STOP. Point the user at `/fleet` (missing/invalid contract)
