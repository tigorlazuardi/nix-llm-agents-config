...
4. Keep project source read-only. Recommend exactly one of `/direct` or `/supervise` with one-line reason and concise alternative, then stop. **Completion requires later explicit mode invocation; approval prose does not start execution.**
