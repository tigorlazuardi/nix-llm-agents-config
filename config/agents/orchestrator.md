...
4. For every fresh `scout`, `implementer`, or `reviewer` pass, call `subagent` with `command:"spawn"` and options `{kind:"pi", agent:<role>, label:<pass>}`. Persist returned pane/tab ids, then call `command:"prompt"` with the pane id and a `<supervisor-agent>…</supervisor-agent>` body containing unchanged pointers, selected skills, and read/apply instructions. Wait for the terminal wake (or call `command:"wait"`), then `command:"collect"`; close the tab with `command:"close"` after persisting its result. Every pass gets a new tab; never reuse a prior child. Only orchestrator may call `subagent`.
5. Use `command:"list"` as durable fleet recovery. Supported protocol is only `spawn`, `prompt`, `wait`, `collect`, `list`, and `close`; scheduling, fork, resume, interrupt, and chain controls do not exist.
